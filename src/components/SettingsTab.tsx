...
import { Edit2, Layers, Database, X, DollarSign, Briefcase, RefreshCw } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { DepositModal } from './DepositModal';
import { AddAssetForm } from './AddAssetForm';
import { EditPortfolioView } from './EditPortfolioView';
import { BackupView } from './BackupView';
import { invalidateClientQuotesCache } from '../services/portfolioService';
...
export const SettingsTab: React.FC<SettingsTabProps> = ({
  ...
  const [clearStatus, setClearStatus] = useState<string>('Limpar cache & forçar novos dados');
  ...
  const handleClearCache = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('home_chart_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach((k) => localStorage.removeItem(k));

    setClearStatus('Cache limpo com sucesso!');
    if (onSyncComplete) onSyncComplete();
    setTimeout(() => {
      setClearStatus('Limpar cache & forçar novos dados');
    }, 2500);
  };

  return (
    <div className="w-full h-full relative overflow-hidden flex-1 flex flex-col">
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-center items-center gap-3">
              ...
              <button
                ...
                className="w-full max-w-xs min-h-[48px] bg-slate-50 border border-slate-200 rounded-2xl flex items-center gap-4 px-5 active:scale-98 transition-transform shadow-xs cursor-pointer text-slate-600 hover:text-slate-900 mt-2"
              >
                <RefreshCw className="w-4 h-4 text-slate-500 stroke-[2]" />
                <span className="font-medium text-xs">{clearStatus}</span>
              </button>
            </div>
          </motion.div>
        )}
