...
export const EditPortfolioView: React.FC<EditPortfolioViewProps> = ({
  ...
  const toggleAssetExpand = (ticker: string) => {
    const willExpand = !expandedTickers[ticker];
    ...
    // Quando o balão se abre, recolhe de imediato os dados em direto
    if (willExpand && onSyncComplete) {
      onSyncComplete();
    }
  };
                  ...
                  >
                    ...
                    {purchases.length === 0 && !isAddingToThis ? (
                      ...
                      purchases.map((purchase) => {
                        ...
                        // Calcular custo real em EUR para a compra:
                        // Se o ativo for em moeda estrangeira e a compra foi salva em moeda nativa (ex: USD), converte para EUR
                        let effectiveUnitCostEur = purchase.priceEur || purchase.price || pos.currentPrice || 0;
                        if (pos.nativeCurrency && pos.nativeCurrency !== 'EUR' && pos.fxRateToEur && pos.fxRateToEur > 0) {
                          // Se priceEur for idêntico ao preço nativo ou não existir
                          if (!purchase.priceEur || (purchase.price && Math.abs(purchase.priceEur - purchase.price) < 0.001)) {
                            effectiveUnitCostEur = (purchase.price || purchase.priceEur || 0) * pos.fxRateToEur;
                          }
                        }
                        const totalAportado = purchase.totalCostEur !== undefined && purchase.totalCostEur > 0
                          ? purchase.totalCostEur
                          : purchase.shares * effectiveUnitCostEur;
